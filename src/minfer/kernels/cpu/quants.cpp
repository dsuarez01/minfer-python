...
void _dequant_row(
    ...
    switch (y.scalar_type()) {
        case torch::kFloat32: {
            ...
            __dequant_row<float>(qtype, x_ptr+row_idx*b, y_ptr+row_idx*k, k);
            break;
        }
        case torch::kFloat16: {
...
void __quant_row_q2_K(const T * __restrict__ x, block_q2_K * __restrict__ y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < QK_K/16; ++j) {
            ...
            scales[j] = make_qkx2_quants<T>(16, 3, x + 16*j, weights, L + 16*j, &mins[j], Laux, -0.5f, 0.1f, 15, true);
...
void __quant_row_q3_K(const T * __restrict__ x, block_q3_K * __restrict__ y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        float amax = 0;
        for (int j = 0; j < QK_K/16; ++j) {
            scales[j] = make_q3_quants<T>(16, 4, x + 16*j, L + 16*j, true);
...
void __quant_row_q4_K(const T * __restrict__ x, block_q4_K * __restrict__ y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < QK_K/32; ++j) {
            ...
            scales[j] = make_qkx2_quants<T>(32, 15, x + 32*j, weights, L + 32*j, &mins[j], Laux, -1.f, 0.1f, 20, false);
            float scale = scales[j];
            if (scale > max_scale) {
                max_scale = scale;
            }
...
void __quant_row_q5_K(const T * __restrict__ x, block_q5_K * __restrict__ y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < QK_K/32; ++j) {
            ...
            float av_x = sqrtf(sum_x2/32);
            for (int l = 0; l < 32; ++l) weights[l] = av_x + fabsf(x[32*j + l]);
            scales[j] = make_qkx2_quants<T>(32, 31, x + 32*j, weights, L + 32*j, &mins[j], Laux, -0.5f, 0.1f, 15, false);
            float scale = scales[j];
            if (scale > max_scale) {
                max_scale = scale;
            }
...
void __quant_row_q6_K(const T * __restrict__ x, block_q6_K * __restrict__ y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int ib = 0; ib < QK_K/16; ++ib) {
            ...
            const float scale = make_qx_quants<T>(16, 32, x + 16*ib, L + 16*ib, 1, NULL);
            scales[ib] = scale;
    ...
    for (int ibl = 0; ibl < nbl; ++ibl) {
        ...
        for (int ib = 0; ib < QK_K/32; ++ib) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                int grid_index = kmap_q2xs[u];
                if (grid_index < 0) {
                    printf("Oops: found point %u not on grid:", u); // what
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("_quant_row", &_quant_row);

    // for cleanup once Python exits
