...
def main():
    ...
    # pruning
    s.add(Or([BM == 64, BM == 128, BM == 256, BM == 512]))
    s.add(Or([BK == 32, BK == 64, BK == 128]))
    s.add(Or([BN == 64, BN == 128, BN == 256, BN == 512]))
    ...
    with open("./tuning/tune.cuh", "w") as f:
        f.write("#pragma once\n\n")
        f.write("#include \"common/types.hpp\"\n\n")
        ...
        for cfg in configs:
            bm, bk, bn, wm, wk, wn, mm, mk, mn, kp, sync = cfg
            f.write(f"\t{{\"float16\", {bm}, {bk}, {bn}, {wm}, {wk}, {wn}, {mm}, {mk}, {mn}, {kp}, {sync}}},\n")
        f.write("};\n\n")
