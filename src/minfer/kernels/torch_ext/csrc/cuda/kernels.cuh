...
struct WmmaConfig {
    ...
    static constexpr size_t SHMEM_SZ = const_max(
        2*TILE_M*(CHUNK_K_VAL*WMMA_K+SKEW_HALF), // X+W tiles (written to shmem as half)
        TILE_M*TILE_N*4                          // out tiles (written to shmem as float)
    );
};

//pre-defined configs
using Config_128x128_k8_T = WmmaConfig<128, 128, 8, 4, 2, true>;  // X @ W.T (more frequently the case)
using Config_128x128_k8 = WmmaConfig<128, 128, 8, 4, 2, false>;   // X @ W
using Config_64x256_k16_T = WmmaConfig<64, 256, 16, 2, 4, true>;  // X @ W.T, W quantized (more frequently the case)
using Config_64x256_k16 = WmmaConfig<64, 256, 16, 2, 4, false>;   // X @ W, W quantized