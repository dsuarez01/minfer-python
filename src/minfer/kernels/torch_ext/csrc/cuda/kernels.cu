...
void rmsnorm_cuda(const at::Tensor& in, at::Tensor& out, const at::Tensor& w, float eps) {
    ...
    TORCH_CHECK(in.size(-1) == w.size(0) || in.size(-1) % w.size(0) == 0);
    ...
    const int n_norms = in.size(0) * in.size(1) * (in.size(-1) / dim);

    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    rmsnorm_cuda_impl<<<n_norms, 1024, 0, stream>>>(in_ptr, out_ptr, w_ptr, dim, eps);
}
