...
void matmul_cuda(
    ...
    if (qtype == GGMLQuantizationType::F16) {
        ...
        if (N >= 16) {
            ...
            dim3 block(BLOCK_SIZE);
            matmul_f16_cuda_impl<TILE_M, TILE_N, TILE_K><<<grid, block, 0, stream>>>(
                M, K, N, x_ptr, out_ptr, w_ptr
            );
        }
    } else {
        ...
        for (int64_t k_tile=0; k_tile<K; k_tile+=TILE_K) {
            ...
            if (blockIdx.y * TILE_N < N_Q) {
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    int nl = idx / TILE_K;
                    int kl = idx % TILE_K;
                    w_shared[nl*TILE_K + kl] = wq[nl*K + k_tile + kl];
                }
            ...
            if (blockIdx.y * TILE_N < N_KV) {
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    int nl = idx / TILE_K;
                    int kl = idx % TILE_K;
                    w_shared[nl*TILE_K + kl] = wk[nl*K + k_tile + kl];
                }
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    int nl = idx / TILE_K;
                    int kl = idx % TILE_K;
                    w_shared[nl*TILE_K + kl] = wv[nl*K + k_tile + kl];
                }
        ...
        for (int64_t k_tile=0; k_tile<K; k_tile+=TILE_K) {
            ...
            if (blockIdx.y * TILE_N < N_Q) {
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    ...
                    const uint8_t* wq_block = wq + (nl*(K/q_qblock_size) + (k_tile/q_qblock_size + kb))*q_qtype_size;
            ...
            if (blockIdx.y * TILE_N < N_KV) {
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    ...
                    const uint8_t* wk_block = wk + (nl*(K/k_qblock_size) + (k_tile/k_qblock_size + kb))*k_qtype_size;
                ...
                for (int idx=threadIdx.x; idx<TILE_N*TILE_K; idx+=blockDim.x) {
                    ...
                    const uint8_t* wv_block = wv + (nl*(K/v_qblock_size) + (k_tile/v_qblock_size + kb))*v_qtype_size;
...
void ffn_cuda(
    ...
    if (
        ...
        // swiglu
        swiglu_f16_cuda_impl<TILE_SIZE, TILE_SIZE, TILE_K, BLOCK_SIZE><<<grid_swiglu, block, 0, stream>>>(
            M, K_GU, N_GU,
        ...
        for (int e=0; e<n_local_exps; e++) {
            ...
            matmul_f16_cuda_impl<TILE_SIZE, TILE_SIZE, TILE_K><<<grid_down_exp, block, 0, stream>>>(
                M, K_DOWN, N_DOWN,
